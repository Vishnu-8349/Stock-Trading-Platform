import React from "react";

function Universe() {
  return (
    <div>
      <div className="container p-5 mt-5">
        <div className="row text-center">
          <h1>The Zerodha Universe</h1>
          <p className="fs-5 mt-3">
            Extend your trading and investment experience even further with our
            partner platforms
          </p>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\smallcaseLogo.png" style={{width: "70%"}}/>
            <p className="text-small text-muted mt-3">
              Thematic investment platform
            </p>
          </div>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\streakLogo.png" style={{width: "60%"}}/>
            <p className="text-small text-muted mt-3">Algo & strategy platform</p>
          </div>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\sensibullLogo.svg" style={{width: "70%"}} className="p-3"/>
            <p className="text-small text-muted mt-3">Options trading platform</p>
          </div>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\zerodhaFundhouse.png" style={{width: "80%"}}/>
            <p className="text-small text-muted mt-3">
              Thematic investment platform
            </p>
          </div>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\goldenpiLogo.png" style={{width: "70%"}}/>
            <p className="text-small text-muted mt-3">Algo & strategy platform</p>
          </div>
          <div className="col-4 p-3 mt-5">
            <img src="media\images\dittoLogo.png" style={{width: "60%"}}/>
            <p className="text-small text-muted mt-3">Options trading platform</p>
          </div>
          <button className='p-3 btn btn-primary fs-5 mb-5 mt-5' style={{width: "25%", margin: "auto"}}>Sign up for free</button>
        </div>
      </div>
    </div>
  );
}

export default Universe;
